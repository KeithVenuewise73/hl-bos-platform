// Deterministic HL-BTI Executive Business Transformation Blueprint assembly
// (Deliverable 4). It composes the STRUCTURE of the executive blueprint from the
// executive scorecard + growth intelligence + recommendations. The AI narrative
// is layered separately (reusing the CP6 blueprint assembler / ai fence) and is
// advisory only — it never changes a score or invents a section's data.
//
// Honesty rule: a section with no supporting data is emitted with
// `hasData: false` and an explicit reason, never a fabricated paragraph.

import type { ExecutiveScorecard, DomainKey } from "./scoring.ts";
import type { GrowthIntelligenceResult } from "./growth.ts";

export const BTI_BLUEPRINT_VERSION = "bti-blueprint-0.1.0";

// The blueprint sections mandated by the PCO, in order.
...
export interface BlueprintSection {
  ...
  note?: string; // why a section is empty, when hasData is false
}
...
export interface ExecutiveBlueprint {
  ...
  complete: boolean; // true only if every mandated section has data
}
...
export interface BlueprintInputs {
  ...
  // recommendations across all domains (already prioritized upstream)
  recommendations: {
    title: string;
    priority: string;
    estimatedRoi?: string;
...
// Deterministic assembly. Sections derive ONLY from real inputs.
export function assembleExecutiveBlueprint(input: BlueprintInputs): ExecutiveBlueprint {
  ...
  const section = (
    ...
    content: Record<string, unknown>,
    note?: string,
  ): BlueprintSection => ({ key, title: TITLES[key], hasData, content, note });
  ...
  return {
    version: BTI_BLUEPRINT_VERSION,
    scores,
    sections,
    complete: sections.every((s) => s.hasData),
  };
}
