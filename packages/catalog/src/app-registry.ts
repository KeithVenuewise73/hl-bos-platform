...
export const APPLICATIONS: ApplicationRecord[] = [
...

  {
    ...
    dependencies: [
      ...
      "public.barberos_clients()",
      "public.barberos_client()",
      "public.barberos_save_client()",
      "public.barberos_record_visit()",
      "public.barberos_clients_due()",
      "public.barberos_barbers()",
      "public.barberos_slots()",
      "public.barberos_book()",
      "public.barberos_day_sheet()",
    ],
    reusableModules: [],
    softwareFactoryIntegration:
    ...
    notes:
      "Holds NO service-role key and cannot hold one usefully: every request is made as the signed-in barber with the publishable key, so RLS and identity.has_permission() decide what happens. It has no authorization logic of its own -- it asks barberos_my_shops() what the person may do and draws its controls from the answer, and an unrecognised permission reads as false. Deliberately WITHOUT the dev-role bypass the Executive Portal has: a bypass that must be correctly guarded is one that can be incorrectly guarded, and this holds a customer's own data. The API it calls is LIVE in HL-BOS Core (0056 applied 2026-09-13, fingerprint-verified, and exercised against production: a signed-in stranger sees no shops and is refused every write). DEPLOYED 2026-09-13 and verified serving from production: /login returns 200 text/html carrying the sign-in form, / redirects an unauthenticated visitor to /login, and the page does NOT carry the 'not pointed at a BarberOS database' warning -- so the build-time public variables were inlined correctly. Its data path was proved end to end beforehand: a real test barber (hl-test-shop) signed in over HTTPS and called the API over PostgREST as themselves, built a page, priced and deleted services and published -- and the page was served at shops.hermanlegacydigital.com as real HTML. The React in front of that path was NOT working when it first shipped and is the most instructive bug in this app's history: a static `script-src 'self'` blocked Next's own inline hydration payload, so the page rendered perfectly, React never attached a single handler, and the sign-in form fell through to a native submit that reloaded /login. A clean build, a green deploy and a 200 carrying the form all agreed it worked. Serving a form is not the same as the form working. Fixed by minting a per-request nonce in middleware and forcing the root layout dynamic, and verified against the live site: two requests return two different nonces, and all fourteen script tags -- including the three carrying `self.__next_f.push` -- are stamped with the one in that response's own header. A repo-wide test now fails any app whose script-src admits neither a nonce nor 'unsafe-inline'. Since 2026-09-13 the app also carries the CLIENT SCREENS behind migration 0058: add a client, record the cut that was actually given, and read a timeline that shows lifetime value next to the visits that have no price recorded. A client with fewer than three visits is shown as having no known rhythm, in words, rather than a guess. THE APPOINTMENT BOOK follows it (migration 0059, built and awaiting approval): a day sheet, a rota, and a two-click booking flow in which every list of free times arrives with the database's own sentence saying why it is what it is -- 'no working hours have been set for this barber yet' is not the same screen as a full day, and rendering both as an empty list is how a shop stops believing the software. The slot list is rendered on the SERVER, so the reason and the times arrive in the same HTML and cannot come apart. The write controls on the client screens are now gated on `manage_clients`, which 0058 shipped without: a viewer was being offered 'Add a client' and refused by the database after pressing it.",
    evidence:
      "apps/barbershop + migrations 0056/0058/0059; 117 pgTAP assertions on the API surface, the client record and the book, 83 unit tests on the mappings, and two end-to-end runs (17 and 26 checks) written as the shop owner through the permission-checked functions",
  },
