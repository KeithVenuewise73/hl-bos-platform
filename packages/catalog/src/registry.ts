...
const ASSETS: Asset[] = [
  ...
  ...sharedService(
    ...
    "built_undeployed",
    ["reusable"],
    "Publishes approved content to Herman Legacy-owned channels: Facebook Page, Instagram, LinkedIn member, TikTok inbox. Approval-gated, per-target fan-out, append-only attempt log.",
    ["db.social", "mod.social-publishing"],
  ),
  ...sharedService(
    "discovery",
  ...
  ...moduleAsset(
    ...
    "social",
    "Compose, approve, schedule and fan out to owned social channels. Phase 1: FB Page, Instagram, LinkedIn member, TikTok inbox (draft only).",
    ["svc.social"],
  ),
  ...moduleAsset(
    "storage",
  ...
  ...edgeFn(
    ...
    "Social Token Refresh",
    "Daily cron. Refreshes any social credential expiring within 14 days and alerts loudly on failure — the silent expired token is the most common production failure in these integrations.",
    ["svc.social"],
    [],
    "built_undeployed",
  ...
  ...edgeFn(
    ...
    "Social Publish Worker",
    "Minute cron. Claims due targets with FOR UPDATE SKIP LOCKED, runs one adapter per platform, logs every attempt. Per-target outcomes are independent.",
    ["svc.social"],
    ["svc.social"],
    "built_undeployed",
    ["Meta", "LinkedIn", "TikTok"],
  ...
  },
  {
    ...
    summary:
      "Provider abstractions (ai/billing/comms/discovery/blueprint/bti/hlvs/provisioning/sales/social/storage) + the outbox dispatcher pattern + vault-by-reference.",
    maturity: "built_undeployed",
    reuse: ["reusable"],
    owner: "Herman Legacy Platform",
    layer: "HL-BOS",
  ...
  ...db(
    ...
    6,
    "HL-BOS",
    "Owned channels, Vault-referenced credentials, posts, per-channel fan-out, append-only publish attempts, media.",
  ),
  ...db(
    "discovery",
    "Discovery",
  ...
  ...db(
    "barberos",
    "BarberOS Capability Spine + owned_website + client_crm + booking",
    19,
    "HL-BOS",
    "Module catalog, prerequisites, bundles, per-tenant capability toggle, the shop record, and the two modules behind that toggle: owned_website (a page the shop owns -- hours, services, prices, address, tap-to-call, booking link) client_crm (the client, their visits, the cut that was actually given, and the rhythm they come back on) and booking (who works when, what times are free, and an appointment book in which two people can never hold the same chair), each gated table-by-table by a trigger on the capability. Thirteen of the sixteen catalogued modules remain 'planned' or 'deferred' and cannot be enabled.",
    "live",
    "supabase/migrations 0048/0052/0053/0056/0057/0058 APPLIED to HL-BOS Core (each fingerprint-verified against the local build); 0059 (booking) is BUILT AND NOT APPLIED, awaiting approval; the public read path is live and answering as role anon; the ONLY tenant with any capability enabled is our own test shop (hl-test-shop, whose page says on its face that it is not a real barbershop) -- it has owned_website and client_crm, its page is currently `unpublished` so nothing is served publicly, and it holds zero clients and zero visits. No customer shop exists",
  ),
  ...db(
    "transform_audit",
