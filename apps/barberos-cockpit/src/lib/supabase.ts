...
// permission-checked public.barberos_* / public.barberos_audit_* RPCs are. The
// signed-in operator's JWT rides on every request, so
// identity.has_permission(tenant, …) governs everything, including which
// agency's pipeline is readable and which shop's capabilities can be changed.
//
