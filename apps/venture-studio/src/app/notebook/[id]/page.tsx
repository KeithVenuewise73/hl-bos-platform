...
import { Card, Empty, Badge, Row, colors } from "@/components/ui";
import { ProvisioningBanner } from "@/components/sections";
import { NotebookStatusControl } from "@/components/Forms";
import { getNotebookEntry } from "@/lib/notebook";
  ...
  return (
    <StudioShell view="notebook_detail">
      <p style={{ fontSize: 12.5, marginBottom: 8 }}>
        <a href="/notebook" style={{ color: colors.accent, textDecoration: "none" }}>
          ← CEO Notebook
        </a>
      </p>
              ...
              <Row
                ...
                v={
                  obj.entry.opportunityId ? (
                    <a
                      href={`/opportunities/${obj.entry.opportunityId}`}
            ...
            {obj.facets.ceoNotes.available ? (
              obj.facets.ceoNotes.data.map((n) => (
                <Row
                  key={n.id}
                  k={
                    <a href={`/notebook/${n.id}`} style={{ color: colors.accent }}>
