 ...
 *
 * The ATS Resume Optimizer was built local-first: no login, data in a JSON file
 ...
 * a career database is a resume, a salary-adjacent scope history, and a record
 * of every job you were rejected from. Deployed without a login, all of it is
...
export function describeMode(mode: DeploymentMode): string {
  switch (mode) {
    ...
    case "refuse":
      return "This app is running in a deployed environment with no identity provider configured. It is refusing to serve, because starting without a login would publish a career database to anyone who found the address.";
    default:
      return "";
  }
}
