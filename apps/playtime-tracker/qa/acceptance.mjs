...
function check(name, actual, expected) {
  ...
  if (!pass) failures += 1;
  results.push(`${pass ? "ok  " : "FAIL"}  ${name}${pass ? "" : `\n        expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`}`);
  return pass;
}

function checkThat(name, condition, detail = "") {
  if (!condition) failures += 1;
  results.push(`${condition ? "ok  " : "FAIL"}  ${name}${condition ? "" : `\n        ${detail}`}`);
  return condition;
}
...
checkThat("app loads and shows the empty state", await page.locator(".empty h3").innerText() === "No teams yet");
...
// --- 3. Add 20 players -----------------------------------------------------
const NAMES = ["Dominic Herman","Marcus Reed","Eli Vance","Noah Pike","Owen Hale","Liam Foss",
  "Caleb Nunez","Jonah Reyes","Miles Darcy","Asher Quinn","Levi Barnes","Ezra Cole",
  "Silas Ward","Rowan Tate","Felix Marsh","Jasper Kane","Hugo Lane","Otis Pryor",
  "Rhys Calder","Emmett Shaw"];
...
checkThat("game setup shows regulation time", (await page.locator("text=48 minutes of regulation time.").count()) === 1);
...
for (let i = 0; i < 11; i++) await page.locator("button.player:not(.on)").first().click();
...
await advance(120);
check("clock reads two minutes", await page.locator(".clock .time").innerText(), "2:00");
...
check("clock reads ten minutes after four substitution rounds", await page.locator(".clock .time").innerText(), "10:00");
...
check("twelve backgrounded minutes are still counted", await page.locator(".clock .time").innerText(), "22:00");
...
checkThat("no errors while connected", errors.length === 0, errors.slice(0, 5).join(" | "));

await context.setOffline(true);
await page.locator("button.player.on").first().click();
await page.locator("button.player:not(.on)").first().click();
await advance(120);
check("substitutions still register with no network", await page.locator(".clock .time").innerText(), "24:00");
check("eleven on the field after an offline substitution", await page.locator("button.player.on").count(), 11);
...
await page.waitForTimeout(100);
check("relaunching the app shows the identical clock", await page.locator(".clock .time").innerText(), clockBeforeKill);
check("the field is intact after a relaunch", await page.locator("button.player.on").count(), 11);
...
await page.waitForTimeout(60);
check("halftime does not advance the game clock", await page.locator(".clock .time").innerText(), "24:00");
await page.getByRole("button", { name: "START QUARTER 2" }).click();
await advance(240);
check("the clock resumes for the second quarter", await page.locator(".clock .time").innerText(), "28:00");

// --- 12. Pause and resume --------------------------------------------------
await page.getByRole("button", { name: "PAUSE" }).click();
await page.clock.fastForward(3 * 60 * 1000);
await page.waitForTimeout(60);
check("a paused clock does not advance", await page.locator(".clock .time").innerText(), "28:00");
checkThat("the app says the clock is stopped", (await page.locator("text=The clock is stopped").count()) === 1);
await page.getByRole("button", { name: "RESUME" }).click();
await advance(120);
check("resuming restarts the clock", await page.locator(".clock .time").innerText(), "30:00");

// --- 13. Confirmation before ending the game -------------------------------
await page.getByRole("button", { name: "END GAME" }).click();
checkThat("ending the game asks first", (await page.locator("text=End this game?").count()) === 1);
...
checkThat("the game clock in the report matches the tracker", reportText.includes("Game time tracked: 30:00"),
  reportText.split("\n").slice(0, 8).join(" | "));
checkThat("the report carries the participation percentages", /Game participation: \d+%/.test(reportText));
checkThat("the report states the target is the user's own", reportText.includes("not a league ruling"));
checkThat("minimum status is shown per athlete", /Status: (Minimum met|Below minimum)/.test(reportText));

const below = (reportText.match(/Status: Below minimum/g) ?? []).length;
const met = (reportText.match(/Status: Minimum met/g) ?? []).length;
checkThat("every athlete is classified against the 25% target", below + met === 20, `met=${met} below=${below}`);
...
checkThat("the finished game appears in history", (await page.locator("text=vs West Seneca").count()) >= 1);
const historyLine = await page.locator(".card .muted").last().innerText();
checkThat("history summarises the minimum outcome", /minimum/i.test(historyLine), historyLine);

// --- 16. Empty / offline / honesty states ----------------------------------
await page.goto("/account/");
await page.waitForSelector("h1:has-text('Account')");
checkThat("the account screen states this build has no account service",
  (await page.locator("text=Not available in this build").count()) === 1);
checkThat("erasing this device is reachable in-app",
  await page.getByRole("button", { name: "Erase everything on this device" }).isVisible());
await page.goto("/legal/privacy/");
checkThat("the privacy policy renders", (await page.locator("h1:has-text('Privacy Policy')").count()) === 1);
await page.goto("/legal/terms/");
checkThat("the terms render", (await page.locator("h1:has-text('Terms of Use')").count()) === 1);
await page.goto("/support/");
checkThat("the support page renders", (await page.locator("h1:has-text('Support')").count()) === 1);

await page.goto("/");
await page.waitForTimeout(600);   // let the service worker finish precaching
await context.setOffline(true);
await page.reload();
await page.waitForSelector("h2:has-text('My teams')", { timeout: 10000 }).catch(() => {});
checkThat("the app opens from a cold start with no network",
...
checkThat("an unvisited route still opens offline",
  (await page.locator("h1:has-text('Support')").count()) === 1);

await context.setOffline(false);

checkThat("no uncaught JavaScript errors in the whole run",
  pageErrors.length === 0, pageErrors.slice(0, 5).join(" | "));
...
const unexplained = errors.filter((e) => !/ERR_INTERNET_DISCONNECTED|Failed to fetch|NetworkError/i.test(e));
checkThat("every remaining browser error is an expected offline one",
  unexplained.length === 0, unexplained.slice(0, 5).join(" | "));
