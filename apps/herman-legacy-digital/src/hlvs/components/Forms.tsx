...
export function NewOpportunityForm() {
  const { busy, msg, post } = useSubmit("/HLVS/api/opportunities");
...
export function NotebookEntryForm({
  ...
  const { busy, msg, post } = useSubmit("/HLVS/api/notebook", () => location.reload());
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const f = new FormData(e.currentTarget);
    const g = (n: string): string => {
...
export function NotebookStatusControl({
  ...
  const { busy, msg, post } = useSubmit(`/HLVS/api/notebook/${id}/status`, () =>
    location.reload(),
  );
  return (
    <div>
