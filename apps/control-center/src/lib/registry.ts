...
export const PORTFOLIO: readonly Product[] = [
  ...
  },
  {
    ...
    stage: "development",
    status:
      "Capability spine LIVE in HL-BOS Core (applied 2026-09-09), plus its first real module, LIVE since 2026-09-10. `owned_website` -- a page the shop owns, with hours, services, prices, address, tap-to-call and a booking link -- is applied to production (migration 0052) and is the only capability in the catalog marked 'available'. Every table in it is gated on the capability switch by a database trigger, so turning the capability off makes the rows unwritable rather than merely hidden. The pages are now SERVED, live: migration 0053 and the `site` edge function are both in production (the first edge function this platform has deployed). A slug returns the shop's page; robots.txt and a sitemap let search engines find it; only published pages are readable, and a draft is indistinguishable from a slug nobody has used. BLOCKING A REAL WEBSITE: on the shared supabase.co domain the gateway rewrites HTML to text/plain, so a browser shows the page's source instead of the page. Supabase documents this and says a custom domain is the fix -- but a project accepts only ONE custom domain ever, which would permanently rule out a shop owning its own, the audit finding this module exists to answer. So the pages will be served by a web host instead; the page server was built portable and moving it is a deployment change, not a rewrite. The console probes the live address and says which of these is true right now. Also still true: no shop has the capability enabled or a page, so no real page has been served yet.",
    version: null,
    location:
