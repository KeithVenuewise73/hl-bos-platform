...
export const PORTFOLIO: readonly Product[] = [
  ...
  },
  {
    ...
    stage: "development",
    status:
      'All of it is now in this repository. The shop, its owned website and the client book (14 tables behind 17 RPCs) plus the pre-sale diagnostic, the discovery call and the proposal (10 more tables) were rebuilt into six migrations on 2026-09-17 and checked against the live database piece by piece until the two matched exactly. 151 tests now cover them. Nothing was applied: production already had all of it. Two days ago none of this had a file, a test or a way to rebuild it. What works: a shop and service catalogue, a website that can publish and unpublish, a client book, an engine that learns a client\'s usual haircut interval and ranks who is overdue, and an audit engine that will not let a proposal offer something we have not built. What does not: the client book is empty (0 clients, 0 visits), the only shop is "Herman Legacy Test Shop" and its page is unpublished, no BarberOS app exists here for those 17 RPCs to have a caller, and the 40 audit runs against 50 real Western New York barbershops cannot be read by anything, because the audit schema has no public API \u2014 every one of those runs is also inferred with no evidence and a score of 0, since nothing is deployed and the diagnostic has never actually fetched a website.',
    version: null,
    location:
      "this repository (supabase/migrations 0049\u20130054), mirroring what canonical production already runs",
  },
  {
    name: "SalonAI",
    stage: "not-started",
