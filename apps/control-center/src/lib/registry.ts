...
export const PORTFOLIO: readonly Product[] = [
  ...
  },
  {
    ...
    stage: "development",
    status:
      "Backhaul load matching for small carriers, ANYWHERE IN THE US. The engine filters every truck/load pair on hard constraints (equipment vs commodity, weight and cubic yards, deadhead, service radius, pickup/delivery windows with hours-of-service rests) and ranks what survives by projected contribution and revenue per mile, using a visible formula the dispatcher can adjust. Nothing is tied to a region: trucks and loads are located by city, ZIP or coordinates, resolved on the server from a national ZIP dataset (42,249 ZIPs, all 50 states); a place that does not resolve is refused, never guessed. Times must carry a UTC offset and trips across time zones are labelled. Dedicated-lane equipment is set aside with a reason, never ranked. Equipment types are data, not code, and every record is tenant-scoped. 53 engine tests + 6 app tests; the tenant guard was verified by removing it and watching its test fail. Two sample fleets (Western NY bulk; Dallas regional with a truck starting in Mountain time) plus 'Your own fleet', which holds only what is imported by CSV. Verified in a real browser 22/22, and it starts from this console's Start button. NOT BUILT: road miles are estimated (great-circle x 1.2) because no routing API is connected; no load board is connected; nothing is saved (imports last for the browser tab); no return-load probability is ever shown, because there is no real lane history to base one on.",
    version: null,
