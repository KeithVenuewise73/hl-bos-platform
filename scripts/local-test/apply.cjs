...
(async () => {
  const admin = new pg.Client({ host:'/tmp', port:5433, user:'postgres', database:'postgres' });
  await admin.connect();
  ...
  const files = fs.readdirSync('/tmp/pgtest').filter(f=>/^\d{14}_hlbos_.*\.sql$/.test(f)).sort();
  for (const f of files) {
    const sql = fs.readFileSync(path.join('/tmp/pgtest',f),'utf8');
    try { await c.query(sql); console.log('APPLIED  ' + f); }
    catch(e){ console.log('FAILED   ' + f); console.log('  ' + e.message); if(e.position) console.log('  at char ' + e.position); await c.end(); process.exit(1); }
  }
  await c.end();
