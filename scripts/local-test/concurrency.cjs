...
const fs = require("fs");
const conn = () =>
  new pg.Client({ host: "/tmp", port: 5433, user: "postgres", database: "hlbos" });
...
(async () => {
  ...
  const race = async (label) => {
    ...
    try {
      await c.query("begin");
      await c.query(
        `select set_config('request.jwt.claims', json_build_object('sub','${invitee}','role','authenticated')::text, true)`,
      );
  ...
  const inv = await check.query(
    "select status::text as s from identity.invitations where token_selector = split_part($1,'.',1)",
    [tok],
  );
  await check.end();
  ...
  const pass =
    succeeded === 1 && failed === 1 && n === 1 && inv.rows[0].s === "accepted";
  console.log(
    pass
