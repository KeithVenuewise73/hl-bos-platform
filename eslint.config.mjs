...
export default tseslint.config(
  {
    ignores: [
      ...
      // Capacitor-generated native projects: Gradle, Java, Swift and the web
      // assets Capacitor copies into them. Generated by a toolchain that owns
      // them, not source we write.
      "apps/playtime-tracker/android/**",
      "apps/playtime-tracker/ios/**",
    ],
  },
  ...
  // Standalone Node scripts (governance/CI helpers, per-app build and QA
  // tooling) run in Node, not the browser or the TS build. Give them Node
  // globals so no-undef does not fire on process/console. They still get the
  // security lint via the shared rules.
  {
    files: [
      "scripts/**/*.{mjs,js}",
      "apps/*/scripts/**/*.{mjs,js}",
      "packages/*/scripts/**/*.{mjs,js}",
    ],
    languageOptions: {
      globals: {
        process: "readonly",
  ...
  // Browser-driving QA harnesses. They run in Node, but the callbacks they
  // hand to page.evaluate() are browser code, executed in the page -- so both
  // sets of globals are legitimately present in one file.
  {
    files: ["apps/*/qa/**/*.{mjs,js}"],
    languageOptions: {
      globals: {
        process: "readonly",
        console: "readonly",
  ...
  // PlayTime Tracker's env boundary, for the same reason as the two above: it
  // is a Next.js static export, so a NEXT_PUBLIC_ value is only inlined into
  // the browser bundle when it is read as LITERAL dot-access. Routing these
  // through @hl-bos/config's loadEnv() would leave every one of them undefined
  // on a device and the app would quietly decide it has no backend. Both
  // Supabase values are browser-safe by ENV_SPEC (RLS is the boundary, not
  // secrecy). Scoped to this one file to keep the guard everywhere else.
  {
    files: ["apps/playtime-tracker/src/lib/config.ts"],
    rules: {
      "no-restricted-properties": "off",
      "no-restricted-syntax": "off",
    },
  ...
  // Service workers. Not Node and not a page: they run in a worker global
  // scope with its own set of names (self, caches, fetch, Response). Listing
  // them is what lets the offline shell be linted at all rather than excluded.
  {
